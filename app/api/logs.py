...
def _require_api_key(x_api_key: str | None = Header(default=None)) -> None:
    """ Dependency: enforce X-API-Key when AUTOTRIAGE_API_KEY is configured. If the env var is unset the endpoint is open (dev/internal mode). """
...
@router.post(
    "/logs", response_model=LogIngestResponse, status_code=201, dependencies=[Depends(_require_api_key)],
)
def ingest_log(
    ...
    """ Ingest a single structured API error log. Triage runs asynchronously in the background; poll GET /api/v1/logs/{id} for results. Requires X-API-Key header when AUTOTRIAGE_API_KEY env var is set. """
    record = ErrorLog(
...
def retriage_log(
    ...
    """Re-run triage analysis on a stored log"""
    record = db.query(ErrorLog).filter(ErrorLog.id == log_id).first()
    if record is None:
        raise HTTPException(status_code=404, detail="Error log not found")
    # ... (rest of function implementation remains unchanged)